...
def main(atoms, dbname, T=500, steps=20000):
    ...
    pwd = os.getcwd()
    os.chdir('../../mtp-training')
    path = os.getcwd()
    os.chdir(pwd)
    mtp_file = 'pot.mtp'
    calc = MTP(f"{path}/{mtp_file}", unique_elements=elements)
    db = connect(dbname)
    ...
    if db.count() == 0:
        ...
        # make sure only the 1-nn is swapped
        r = 1.4
        ...
        while attempt < steps:
            ...
            while ind1 is None:
                ...
                new_atoms.calc = MTP(f"{path}/{mtp_file}", unique_elements=elements)
